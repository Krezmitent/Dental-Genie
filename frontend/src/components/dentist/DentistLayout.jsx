...
const DentistLayout = () => {
  ...
  const navItems = [
    { name: 'Dashboard', path: '/dentist/dashboard', icon: 'dashboard' },
    { name: 'Appointments', path: '/dentist/appointments', icon: 'calendar_month' },
    { name: 'AI Reviews', path: '/dentist/reviews', icon: 'fact_check' },
    { name: 'My Patients', path: '/dentist/patients', icon: 'groups' },
  ];
    ...
    <div className="min-h-screen bg-layer-0 flex font-body text-on-surface">
      ...
      >
        ...
        <div className="px-4 py-6 border-b border-outline-variant/20 flex items-center space-x-3 bg-surface-container/30">
          <div className="w-10 h-10 rounded-full bg-surface-container border border-outline-variant flex items-center justify-center text-on-surface font-bold text-lg shadow-[0_0_10px_rgba(0,0,0,0.5)]">
            {user?.name?.charAt(0).toUpperCase()}
          </div>
          <div className="overflow-hidden">
            <p className="text-sm font-medium text-on-surface truncate">Dr. {user?.name}</p>
            <p className="text-[11px] font-mono text-primary-fixed-dim/80 truncate uppercase tracking-wider">{user?.profile?.specialization || 'Dentist Portal'}</p>
          </div>
        </div>
              ...
              className={({ isActive }) =>
                `flex items-center px-3 py-3 rounded-xl text-sm font-medium transition-colors border ${
                  isActive
                    ...
                    : 'text-on-surface-variant border-transparent hover:bg-surface-container-high hover:text-on-surface hover:border-outline-variant/30'
                }`
              }
            >
